#!/usr/bin/env bash
# portal-golive — MANUAL production deploy to the CURRENT origin/main, after typed confirmation.
# Installed as /usr/local/bin/portal-golive.
. /usr/local/bin/portal-lib.sh || exit 1
[ "$PORTAL_ENV" = "production" ] || { echo "REFUSING: golive is production-only (this host is env='$PORTAL_ENV')."; exit 1; }
[ -t 0 ] || { echo "golive must be run interactively (a TTY is required)."; exit 1; }

# Acquire the deploy lock BEFORE fetch/classify so nothing shifts between confirm and deploy.
exec 9>"$DEPLOY_LOCK"
flock -n 9 || { echo "another deploy is running; try again shortly."; exit 1; }

cd "$PORTAL_DIR" || exit 1
echo "Fetching latest from GitHub..."; do_fetch || { echo "fetch failed"; exit 1; }
classify_state
case "$STATE" in
...
new=$(git rev-parse origin/main)          # PIN the exact sha we show, confirm, and deploy
...
guarded_deploy "$new" && echo "go-live complete." || { echo "go-live FAILED — see the alert / logs."; exit 1; }
