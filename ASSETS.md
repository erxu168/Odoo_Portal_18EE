...
| `useConfirm` | `ui/useConfirm.tsx` | **Promise-based `ConfirmDialog`** — the drop-in for the browser's `confirm()`. `if (!await confirm({title, message, confirmLabel, variant:'danger'})) return;` keeps the original control flow, so converting a delete path is a one-line change instead of a rewrite. Render `confirmElement` once. Refuses a second dialog while one is open, and fails CLOSED on unmount | 6 |
| `DrawingLayer` | `ui/DrawingLayer.tsx` | Vector overlay over any image: arrow / circle / box / freehand pen **/ ⚠ warning stamp / text typed onto the photo**, 4 colours, **5 line weights** (which set a stamp's SIZE, since it has no stroke), select → move → stretch → erase. Coordinates are fractions 0..1 so a mark lands identically at any size; the photo bytes are never touched. Shapes + helpers live in `lib/guide-drawings.ts` (client-safe, no server imports) | 1 (via `PinnableImage`) |
| `DrawingToolbar` + `useDrawingTools` | `ui/DrawingToolbar.tsx` | The mark-up controls for an annotated photo — tool, 4 colours, 5 weights, erase / undo / clear — and the state behind them. Feed `useDrawingTools` straight to `PinnableImage`'s `draw*` props. `allowPins` says whether "Dots" (numbered note-pins) exists; without it a drawing tool is active from the start, or dragging would do nothing. **Building anything draw-on-a-photo? Read the `annotated-photo` skill first** | 2 |
| `AnnotatedPhotoThumb` | `ui/AnnotatedPhotoThumb.tsx` | A small photo you tap to see full size, marks and all. Owns the failed-to-load state (a bare `<img>` on a 404 shows the browser's broken-image icon, which reads as "this app is broken") and opens the enlarged view through a **portal** — rendered in place, its backdrop is a child of the row, and closing the picture fired the row's "this is done" handler | 2 |
| `RowMenu` | `ui/RowMenu.tsx` | The "⋮" menu on a list row — Edit / Delete / anything else. Keeps a permanently-red Delete off every row and gives the row its width back. Closes on outside tap and Escape; every handler stops the click, because these live inside rows that are themselves tappable | 1 |
| `writeGuide` | `lib/ai/guide-writer.ts` | Photos + rough notes → a DRAFT guide with steps and quiz questions. Returns a draft and nothing else — it cannot save or publish, so no AI text reaches a cook unread. Strict JSON-schema output, hard caps on photos/notes/questions, and a prompt that forbids inventing temperatures, times or safety limits not in the notes. Degrades to `not-configured` with no API key, like `music/classifier.ts` | 1 |
...
| `yield.ts` | `lib/yield.ts` | **What a kilo really costs once the peel is binned** — pooled yield %, true cost, and kg-per-piece from `raw in / pieces / usable out` weighings. **THE RULE: a weight base unit does NOT make a pack size measurable.** It rules out "1 crate = 20 bottles", but not "Ketchup 10kg Eimer" or "Carrots Sack 10 Kg" — real kg rows whose pack weight the SUPPLIER declares. Three underfilled buckets would otherwise "prove" a correct 10 kg is 9.85 and skew every future count. So a human answers `pack_varies` once per product before any offer appears, and the offer uses RAW kg/piece (what staff see on the shelf), never usable. Averages POOL totals over the 10 most recent tests — the mean of the ratios would let a 400 g test outweigh a 20 kg one. Applying a size rescales every company's par in the same transaction, so "ten bunches" stays ten bunches |
...
| `print-errors.ts` | `lib/print-errors.ts` | **Why a label did not print, in words a cook can act on** — `explainPrintFailure(raw)` and `printFailure(seq, raw)`. Every label screen used to show a bare "Print failed for label 1": the reason was known one layer down and discarded on the way up, so the only answer anyone could give was "try again". **THE TRAP: `ble.error` is React state and is STALE right after `await ble.print()`** — the handler holds the `ble` object from the render that made it, so that read returns the value from BEFORE the print (always null, which is why LocationLabels always said "the printer stopped responding"). Read `ble.printError()`, a ref. Used by LabelPrint, PackageLabel, ShelfLabelPrint, LocationLabels |
