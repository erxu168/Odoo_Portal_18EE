...
## Changing the app icon (the picture on the home screen)

The app ships with a brand clock icon (white clock on Krawings green). The source
art is in `assets/` here and is generated by `make-icon.py`. To change it:

**Option A — a different design of the same clock** (colours/hands): edit the
colours at the top of `make-icon.py`, then from the app project root:

```bash
pip install pillow                       # once
python3 docs/android-timeclock/make-icon.py   # regenerates assets/*.png
npx @capacitor/assets generate --android      # expands to all Android sizes
# rebuild the APK (Android Studio Run, or ./gradlew assembleDebug)
```

**Option B — your own logo:** replace `assets/icon-foreground.png` with your logo
as a **1024×1024 transparent PNG** (subject centred in the middle ~66% so it isn't
clipped by round/squircle launchers), and set `assets/icon-background.png` to a
solid brand colour (or a 1024×1024 image). Then run the same two commands above +
rebuild. To keep it dead simple you can instead supply a single full-bleed
`assets/icon-only.png` (1024×1024) — the tool will use it for everything.

**Option C — no scripts, all clicks:** in Android Studio, right-click `app` →
**New → Image Asset** → pick your image → Finish. That regenerates every size for you.

> `@capacitor/assets` also refreshes the launch (splash) screen from `assets/splash.png`.
