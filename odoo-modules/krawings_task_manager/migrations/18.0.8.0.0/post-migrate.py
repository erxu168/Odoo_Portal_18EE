...
def _daily_content(cr):
    ...
    must stay byte-for-byte identical across this migration (source/audit refs
    are deliberately excluded; those are what the migration backfills)."""
    cr.execute("""
        SELECT s.id, s.list_line_id, s.sequence, s.media_type,
               s.explanation, s.image_filename, s.pdf_filename, s.youtube_url,
               (SELECT count(*) FROM krawings_task_guide_pin p WHERE p.step_id = s.id)
...
def _daily_completion(cr):
    """Completion + snapshot revision of every daily line that carries a guide —
    the migration must not reopen, complete, or re-version any of them."""
    cr.execute("""
        SELECT ll.id, ll.completed_at, ll.completed_by_id, ll.guide_snapshot_revision
...
def migrate(cr, version):
    ...
    for template_line_id, company_id, line_name, published, revision in stash:
        ...
        # Refresh caches after the raw move, then set published/revision so the
        # invariant re-validates against the now-attached steps, and link the
        # task (recomputes its stored relateds + has_guide).
    ...
    # Verify — daily content + completion must be identical; any drift aborts.
    if _daily_content(cr) != base_content:
        raise Exception(
            "[krawings_task_manager] 18.0.8.0.0 ABORT: a daily guide snapshot's "
            "content changed during migration — rolling the whole upgrade back."
        )
