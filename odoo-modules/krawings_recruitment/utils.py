...
# Where the Next.js portal listens (same box, behind nginx). Both values are
# overridable via Odoo system parameters so we never hardcode environment
# specifics in code:
...
def portal_post(env, path, payload):
    ...
    Returns a ``(status_code, json_body)`` tuple. Transport errors
    (``requests.exceptions.*``) propagate to the caller, which decides how to
    surface them to the user. A missing token raises ``ValueError`` so the
    caller can show a clear configuration message instead of a silent 401.
    """
    ...
    if not token:
        raise ValueError(
            ...
            "KRAWINGS_INTERNAL_API_TOKEN in the portal) before using the "
            "recruitment bridge."
        )
    ...
    _logger.info("[krawings_recruitment] POST %s payload=%s", url, payload)
