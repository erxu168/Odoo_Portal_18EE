...
export default function ManufacturingPage() {
  ...
  function handleDashboardNav(tile: string) {
    if (tile === 'orders') navigate({ type: 'mo-list' });
    else if (tile === 'create') navigate({ type: 'create' });
    else if (tile === 'recipes') navigate({ type: 'mo-list', filter: 'recipes' });
    else if (tile === 'completed') navigate({ type: 'mo-list', filter: 'done' });
  }
  ...
  const Header = ({ title, subtitle, showBack }: { title: string; subtitle?: string; showBack?: boolean }) => (
    <div className="bg-[#1A1F2E] px-5 pt-12 pb-3 relative overflow-hidden">
      ...
      <div className="flex items-center gap-3 relative">
        <button onClick={showBack ? goDashboard : goHome} className="w-9 h-9 rounded-xl bg-white/10 border border-white/10 flex items-center justify-center active:bg-white/20 transition-colors">{showBack ? <BackIcon /> : <HomeIcon />}</button>
        ...
        {showBack && <button onClick={goHome} className="w-9 h-9 rounded-xl bg-white/10 border border-white/10 flex items-center justify-center active:bg-white/20 transition-colors" title="Home"><HomeIcon /></button>}
      </div>
    </div>
  );
      ...
      case 'create':
        return (
          <CreateMo
            onBack={goBack}
            onCreated={(moId) => {
              setHistory([{ type: 'dashboard' }, { type: 'mo-list' }]);
