      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-5">
        <section className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4">
          ...
          <div className="space-y-3">
            ...
            <PrimaryButton busy={saving} onClick={saveHeader} className="w-full">
        ...
        <section>
          <div className="mb-2">
            <p className="text-[var(--fs-xs)] font-bold uppercase tracking-wider text-gray-400 mb-2">Tasks</p>
            {/* The locked create standard, and the same button the two sibling
                screens use (New template / New guide). It was a green text link
                about a third the size of a finger, in the corner of a heading. */}
            <button
              type="button"
              onClick={() => setShowAddLine(true)}
              className="w-full min-h-[48px] rounded-xl bg-green-600 text-white text-[var(--fs-sm)] font-bold flex items-center justify-center gap-2 hover:bg-green-700 active:bg-green-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-green-500"
            >
    ...
    <div className="fixed inset-0 bg-black/40 z-[60] flex items-end sm:items-center justify-center" onClick={handleClose}>
      <div className="bg-white w-full max-w-md rounded-t-2xl sm:rounded-2xl flex flex-col max-h-[90dvh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2 px-5 py-4 border-t border-gray-200 flex-shrink-0 bg-white">
          <button onClick={handleClose} className={`${ds.btnSecondary} flex-1`}>Cancel</button>
          <PrimaryButton
            busy={submitting}
            disabled={photoBusy > 0 || uploadingFile}
            onClick={submit}
            className="flex-1"
          >
