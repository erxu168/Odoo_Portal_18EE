...
export default function PurchasePage() {
  ...
  const OrderGuide = () => {
    ...
    return (<>
      ...
      {cartItemCount > 0 && (<div className="fixed bottom-16 left-0 right-0 max-w-lg mx-auto bg-white border-t border-gray-200 px-4 py-3 z-50"><div className="flex justify-between items-center mb-2"><div><div className="text-[18px] font-extrabold font-mono text-[#1F2933]">&euro;{cartAmount.toFixed(2)}</div><div className="text-[11px] text-gray-500">{cartItemCount} items &bull; shared cart ({locName})</div></div></div><button onClick={() => changeTab('cart')} className="w-full py-3.5 rounded-xl bg-green-600 text-white text-[14px] font-bold shadow-lg shadow-green-600/30 active:bg-green-700 active:scale-[0.975] transition-all">View cart &rarr;</button></div>)}
    </>);
  };
  ...
  const CartView = () => {
    ...
    <div className="px-4 py-3 pb-20">
      {carts.length === 0 ? (<div className="text-center py-16"><div className="text-4xl mb-3">&#128722;</div><div className="text-[15px] font-semibold text-[#1F2933] mb-1">Cart is empty</div><div className="text-[13px] text-gray-500">Go to a supplier and add products.</div></div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-3.5 mb-3"><div className="text-[13px] font-semibold text-[#1F2933] mb-1">Order note</div><textarea value={orderNote} onChange={e => setOrderNote(e.target.value)} placeholder="Add a note for this order..." rows={2} className="w-full text-[13px] text-gray-600 border border-gray-200 rounded-lg px-3 py-2 resize-none outline-none focus:border-green-500" /></div>
        {carts.map(cart => {
              ...
              <div key={item.id} className="py-2.5 border-b border-gray-100 last:border-0">
                ...
                <div className="flex items-center justify-between mt-1.5">
                  <div className="flex items-center">
                    ...
                    <button onClick={() => openCartNumpad(item, cart.supplier_id)} className="w-10 h-8 flex items-center justify-center text-[14px] font-bold font-mono text-[#1F2933]">{item.quantity}</button>
