...
type Screen = 'suppliers' | 'guide' | 'catalog' | 'cart' | 'sent' | 'receive-list' | 'receive-check' | 'history' | 'manage';
...
export default function PurchasePage() {
  ...
  // Fetch suppliers when location changes
    ...
    <div className="px-4 py-3">
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <div className="text-[13px] text-gray-500">An admin needs to add suppliers first.</div>
        </div>
      ) : (
        <>
          {suppliers.map(s => {
                  ...
                  return (
                    <div key={item.id}
                      className={`flex items-center gap-2.5 py-2.5 border-b border-gray-100 last:border-0 ${
                        qty > 0 ? 'bg-orange-50 -mx-3.5 px-3.5 rounded-lg mb-1' : ''
                      }`}>
                      <div className="w-10 h-10 rounded-lg bg-[#F1F3F5] flex items-center justify-center text-[14px] flex-shrink-0">
                        &#128230;
                      </div>
                      <div className="flex-1 min-w-0">
                      ...
                      {qty > 0 ? (
                        <div className="flex items-center flex-shrink-0">
                          <button onClick={() => updateCartQty(item, Math.max(0, qty - 1))}
                            className="w-9 h-9 rounded-lg border border-gray-200 bg-white flex items-center justify-center text-[16px] text-gray-600 active:bg-gray-100">-</button>
                          <button onClick={() => openNumpad(item)}
                            className="w-10 h-9 flex items-center justify-center text-[15px] font-bold font-mono text-[#1F2933]">{qty}</button>
                          <button onClick={() => updateCartQty(item, qty + 1)}
                            className="w-9 h-9 rounded-lg border border-gray-200 bg-white flex items-center justify-center text-[16px] text-gray-600 active:bg-gray-100">+</button>
                        </div>
                      ) : (
                        <button onClick={() => updateCartQty(item, 1)}
                          className="w-9 h-9 rounded-lg bg-orange-500 flex items-center justify-center text-white text-[18px] font-bold shadow-sm active:bg-orange-600 flex-shrink-0">+</button>
                      )}
                    </div>
                  );
                })}
            ...
            onClick={async () => {
              for (const cart of carts) {
                await fetch('/api/purchase/orders', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ cart_id: cart.id, delivery_date: null, order_note: '' }),
                });
              }
              fetchCart();
              setScreen('sent');
            }}
            className="w-full py-3.5 rounded-xl bg-orange-500 text-white text-[14px] font-bold shadow-lg shadow-orange-500/30 active:bg-orange-600 transition-all">
            Send order &rarr;
          </button>
        </div>
      )}
  ...
  const OrderSent = () => (
    <div className="px-4 py-3 flex flex-col items-center pt-16">
      ...
      <div className="text-[13px] text-gray-500 text-center max-w-[280px] leading-relaxed mb-6">
        Your order has been submitted. Check history for status updates.
      </div>
      <button onClick={() => changeTab('order')}
  ...
  const HistoryView = () => (
    <div className="px-4 py-3">
      {orders.length === 0 ? (
        <div className="text-center py-16">
          <div className="text-[15px] font-semibold text-[#1F2933] mb-1">No orders yet</div>
          <div className="text-[13px] text-gray-500">Orders from {locName} will appear here.</div>
        </div>
      ) : (
        <div className="bg-white border border-gray-200 rounded-xl shadow-[0_1px_2px_rgba(0,0,0,0.04)] px-3.5">
          {orders.map(order => {
            ...
            const statusLabel: Record<string, string> = {
              pending_approval: 'Awaiting approval', approved: 'Approved', sent: 'Sent',
              received: 'Delivered', partial: 'Partial', cancelled: 'Cancelled', draft: 'Draft',
            };
            return (
              <div key={order.id} className="flex items-center gap-3 py-3 border-b border-gray-100 last:border-0">
                ...
                <div className="text-right flex-shrink-0">
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-md font-bold ${statusStyles[order.status] || 'bg-gray-100 text-gray-700'}`}>
  ...
  const ReceiveList = () => (
    <div className="px-4 py-3">
      ...
      {pendingDeliveries.length === 0 ? (
        <div className="text-center py-16">
          <div className="text-[15px] font-semibold text-[#1F2933] mb-1">No pending deliveries</div>
          <div className="text-[13px] text-gray-500">Sent orders will appear here when ready to receive.</div>
        </div>
      ) : (
        pendingDeliveries.map(order => (
          <div key={order.id}
            className="flex items-center gap-3 p-3.5 bg-white border border-gray-200 rounded-xl shadow-[0_1px_2px_rgba(0,0,0,0.04)] mb-2.5 active:scale-[0.98] transition-transform cursor-pointer">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[11px] text-gray-500 font-mono mt-0.5">
                {order.odoo_po_name || `#${order.id}`} &bull; {(order as any).lines?.length || '?'} items
              </div>
            </div>
      ...
      <div className="fixed inset-0 bg-black/40 z-[100] flex items-end justify-center" onClick={() => setNumpadOpen(false)}>
        <div className="bg-white rounded-t-[20px] w-full max-w-lg p-5 pb-7" onClick={e => e.stopPropagation()}>
          <div className="text-center pb-4">
            <div className="text-[12px] text-gray-400">{numpadProduct?.product_uom}</div>
            <div className="text-[15px] font-bold text-[#1F2933]">{numpadProduct?.product_name}</div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            {['1','2','3','4','5','6','7','8','9','.','0','del'].map(k => (
              <button key={k} onClick={() => numpadKey(k)}
                className="h-14 rounded-xl border border-gray-200 bg-white text-[20px] font-semibold text-[#1F2933] flex items-center justify-center active:bg-gray-100 font-mono">
                {k === 'del' ? '\u232B' : k}
              </button>
            ))}
          </div>
          <button onClick={confirmNumpad}
            className="w-full py-4 rounded-2xl bg-orange-500 text-white text-[15px] font-bold shadow-lg shadow-orange-500/30">
            Confirm
          </button>
        </div>
      </div>
        ...
        <>
          ...
          {tab === 'catalog' && (
            <div className="px-4 py-3 text-center">
              <div className="text-[15px] font-semibold text-[#1F2933] mt-16 mb-1">Catalog</div>
              <div className="text-[13px] text-gray-500">Browse all products — coming next.</div>
            </div>
          )}
          {tab === 'cart' && <CartView />}
