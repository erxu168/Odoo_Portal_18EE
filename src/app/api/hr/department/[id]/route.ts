/**
 * PATCH /api/hr/department/[id]
 * Manager/admin edit of a department, plus archive/restore (active flag).
 * Company-scoped: managers may only touch their own restaurant(s).
 * There is no DELETE — removal is archiving (active:false); Odoo blocks deleting
 * a department that still has staff.
 */
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Load current company (for scoping). active_test:false so archived depts can be restored.
    const existing = await odoo.searchRead(
      'hr.department', [['id', '=', id]], ['company_id'],
    ...
    if (allowed && (currentCompanyId === null || !allowed.includes(currentCompanyId))) {
      return NextResponse.json({ error: 'You can only manage departments in your own restaurant.' }, { status: 403 });
    }
    ...
    if (allowed && vals.company_id !== undefined && !allowed.includes(Number(vals.company_id))) {
      return NextResponse.json({ error: 'You can only move a department to your own restaurant.' }, { status: 403 });
    }

    await odoo.write('hr.department', [id], vals);
    return NextResponse.json({ success: true, updated: Object.keys(vals) });
  } catch (err: unknown) {
    if (err instanceof AuthError) return NextResponse.json({ error: err.message }, { status: err.status });
    console.error('PATCH /api/hr/department/[id] error:', err);
