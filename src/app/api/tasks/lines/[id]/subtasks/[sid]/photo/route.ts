...
/** A DAILY subtask's reference photo, for staff. Company-scoped, fails closed.
...
export async function GET(
  ...
  try {
    ...
    const subtaskId = parseInt(params.sid, 10);
    if (Number.isNaN(lineId) || Number.isNaN(subtaskId)) {
      return NextResponse.json({ error: 'Invalid id' }, { status: 400 });
    }
    const photo = await subtaskPhoto('list', subtaskId, parseCompanyIds(user.allowed_company_ids), lineId);
