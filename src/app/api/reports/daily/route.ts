...
import { computeDailyBreakdown } from '@/lib/report-compute';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const key = cacheKey('daily', locationId, monthParam);
    const cached = cacheGet<any>(key);
    if (cached) return NextResponse.json({ success: true, data: cached.data, cached: true, computedAt: cached.computedAt });

    const data = await computeDailyBreakdown(location, parseInt(yearStr), parseInt(monthStr));
    cacheSet(key, data, REPORT_TTL.daily);
