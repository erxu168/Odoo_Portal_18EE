...
import { computeMenu } from '@/lib/report-compute';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const key = cacheKey('menu', 'all', monthParam);
    const cached = cacheGet<any>(key);
    if (cached) return NextResponse.json({ success: true, data: cached.data, cached: true, computedAt: cached.computedAt });

    const data = await computeMenu(parseInt(yearStr), parseInt(monthStr));
    cacheSet(key, data, REPORT_TTL.menu);
    ...
    console.error('[reports/menu]', err);
