...
import { computeComparison, type ComparisonType } from '@/lib/report-compute';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const type = (searchParams.get('type') || 'week') as ComparisonType;
    ...
    const key = cacheKey('compare', locationId, `${type}-${refDate}`);
    const cached = cacheGet<any>(key);
    if (cached) return NextResponse.json({ success: true, data: cached.data, cached: true, computedAt: cached.computedAt });

    const data = await computeComparison(location, type, refDate);
    cacheSet(key, data, REPORT_TTL.compare);
