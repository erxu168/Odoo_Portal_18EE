...
const TILES = [
  ...
  {
    id: 'inventory', label: 'Inventory', href: null,
  ...
  {
    id: 'contacts', label: 'Contacts', href: null,
    iconBg: 'bg-indigo-50', iconColor: 'text-indigo-600',
    badgeColor: 'bg-indigo-500',
...
const TASK_STATUS_STYLES: Record<string, { border: string; pill: string; pillText: string }> = {
  overdue:  { border: 'border-l-red-500',    pill: 'bg-red-50',     pillText: 'text-red-600' },
  due_soon: { border: 'border-l-amber-400',  pill: 'bg-amber-50',   pillText: 'text-amber-700' },
  upcoming: { border: 'border-l-indigo-400', pill: 'bg-indigo-50',  pillText: 'text-indigo-600' },
  done:     { border: 'border-l-emerald-400', pill: 'bg-emerald-50', pillText: 'text-emerald-700' },
};
      ...
      {tasks && tasks.items && tasks.items.length > 0 && (
        <div className="px-5 pt-4">
          <div className="flex items-center justify-between mb-2.5">
            <h2 className="text-[15px] font-bold text-gray-900">Shift tasks</h2>
            <button
          ...
          <div className="flex flex-col gap-2 mb-2">
            {tasks.items.map((task: any) => {
              ...
              return (
                <div
                  key={task.id}
                  className={`flex items-center gap-3 px-3.5 py-3 bg-white border border-gray-200 rounded-xl border-l-[3px] ${s.border} ${
                    isDone ? 'opacity-50' : ''
          ...
          {TILES.map((tile) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[12px] font-semibold text-gray-900 text-center px-1 leading-tight">
                  {tile.label}
                </span>
              </button>
            );
