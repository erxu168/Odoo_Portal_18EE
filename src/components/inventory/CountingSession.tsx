...
export default function CountingSession({ sessionId, userRole, onBack, onSubmit }: CountingSessionProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      for (const e of (countRes.entries || [])) { entryMap[e.product_id] = e.counted_qty; }
      setEntries(entryMap);
      ...
      if (productIds.length > 0) {
        ...
        results.forEach(r => { (r.products || []).forEach((p: any) => { if (!seen.has(p.id)) { seen.add(p.id); loadedProducts.push(p); } }); });
      }
  ...
  async function saveCount(productId: number, qty: number | null, uom: string) {
    if (qty === null || qty === undefined) {
      ...
      await fetch('/api/inventory/counts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ session_id: sessionId, product_id: productId, counted_qty: qty, uom }) });
  ...
  function openNumpad(product: any) { setNumpad({ open: true, product }); }
  function handleNumpadSave(value: number | null) {
    if (numpad.product) { saveCount(numpad.product.id, value, numpad.product.uom_id?.[1] || 'Units'); }
  ...
  async function handleSubmit() {
    setSubmitting(true);
    try {
      await fetch('/api/inventory/sessions', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: sessionId, status: 'submitted' }) });
      onSubmit();
  ...
  if (view === 'review') {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <div className="px-4 pt-4">
          <div className="bg-white border border-gray-200 rounded-2xl p-4 mb-3">
            ...
            <div className="h-2 bg-gray-100 rounded-full overflow-hidden mb-3">
              <div className={`h-full rounded-full transition-all ${countedCount === totalCount ? 'bg-green-500' : 'bg-amber-500'}`} style={{ width: `${totalCount > 0 ? (countedCount / totalCount) * 100 : 0}%` }} />
            </div>
          ...
          {uncountedProducts.length > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3.5 mb-3">
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  <p className="text-[13px] font-semibold text-amber-800">{uncountedProducts.length} item{uncountedProducts.length > 1 ? 's' : ''} not counted</p>
                  <p className="text-[11px] text-amber-700 mt-0.5">Uncounted items will be submitted as not counted. You can go back and count them.</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-28">
          {countedProducts.length > 0 && (<>
            ...
            {countedProducts.map((p) => {
              const val = entries[p.id]; const uom = p.uom_id?.[1] || 'Units';
        ...
        {canSubmit && (<div className="fixed bottom-0 left-0 right-0 max-w-lg mx-auto px-4 py-3 bg-white border-t border-gray-200 z-40">
          <button onClick={() => setShowConfirm(true)} disabled={submitting || countedCount === 0} className="w-full py-4 rounded-xl bg-green-600 text-white text-[15px] font-bold shadow-lg shadow-green-600/30 active:bg-green-700 active:scale-[0.975] transition-all disabled:opacity-50">Submit for approval</button>
        </div>)}

        {showConfirm && (<div className="fixed inset-0 z-50 bg-black/50 flex items-end justify-center">
          <div className="bg-white w-full max-w-lg rounded-t-2xl p-5 pb-8">
            ...
            <p className="text-[13px] text-gray-500 mb-1">{countedCount} of {totalCount} items counted.{uncountedProducts.length > 0 && ` ${uncountedProducts.length} item${uncountedProducts.length > 1 ? 's' : ''} will be marked as not counted.`}</p>
            <p className="text-[13px] text-gray-500 mb-5">You will not be able to edit after submitting. A manager will review your count.</p>
            <div className="flex gap-3">
              <button onClick={() => setShowConfirm(false)} className="flex-1 py-3.5 rounded-xl bg-gray-100 text-gray-700 text-[14px] font-semibold active:bg-gray-200">Cancel</button>
              <button onClick={handleSubmit} disabled={submitting} className="flex-1 py-3.5 rounded-xl bg-green-600 text-white text-[14px] font-bold shadow-lg shadow-green-600/30 active:bg-green-700 disabled:opacity-50">{submitting ? 'Submitting...' : 'Yes, submit'}</button>
            </div>
          </div>
        </div>)}
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <BackHeader onBack={onBack} title={session?.template_name || `Session #${sessionId}`} subtitle={`${session?.scheduled_date || ''} ${locationName ? '\u00B7 ' + locationName : ''} \u00B7 ${totalCount} products`} />
      ...
      {categories.length > 1 && (<FilterBar>
        ...
        {categories.map((c) => (<FilterPill key={c.id} active={catFilter === String(c.id)} label={c.name} onClick={() => setCatFilter(String(c.id))} />))}
      </FilterBar>)}
      <CountProgress counted={countedCount} total={totalCount} />

      <div className="flex-1 overflow-y-auto px-4 pb-28">
        {totalCount === 0 ? (<EmptyState title="No products configured" body="This counting list has no products. Ask your manager to edit the template." />
        ) : filtered.length === 0 ? (<EmptyState title="No products match" body="Try a different filter or search term" />
        ) : (<div className="flex flex-col">
          {filtered.map((p) => {
            const val = entries[p.id] ?? null; const uom = p.uom_id?.[1] || 'Units'; const catName = p.categ_id?.[1] || '';
            return (<div key={p.id} className="flex items-center gap-3 py-3 border-b border-gray-100">
              ...
              {!isReadOnly ? (<Stepper value={val} uom={uom} onMinus={() => stepQty(p, -1)} onPlus={() => stepQty(p, 1)} onTap={() => openNumpad(p)} />
              ) : (<div className="text-[15px] font-mono font-semibold text-gray-700">{val !== null ? val : '--'} <span className="text-[11px] text-gray-400">{uom}</span></div>)}
            </div>);
          })}
        </div>)}
      </div>

      {canSubmit && countedCount > 0 && (<div className="fixed bottom-0 left-0 right-0 max-w-lg mx-auto px-4 py-3 bg-white border-t border-gray-200 z-40">
        <button onClick={() => setView('review')} className="w-full py-4 rounded-xl bg-green-600 text-white text-[15px] font-bold shadow-lg shadow-green-600/30 active:bg-green-700 active:scale-[0.975] transition-all">Review count ({countedCount}/{totalCount})</button>
      </div>)}

      {isReadOnly && (<div className="fixed bottom-0 left-0 right-0 max-w-lg mx-auto px-4 py-3 bg-gray-100 border-t border-gray-200 z-40">
        <p className="text-center text-[13px] text-gray-500 font-semibold">{session?.status === 'submitted' ? 'Submitted \u2014 awaiting review' : session?.status === 'approved' ? 'Approved' : 'Rejected'}</p>
      </div>)}

      {!isReadOnly && (<NumpadModal open={numpad.open} productName={numpad.product?.name || ''} category={numpad.product?.categ_id?.[1] || ''} uom={numpad.product?.uom_id?.[1] || 'Units'} initialValue={numpad.product ? (entries[numpad.product.id] ?? null) : null} showSystemQty={userRole !== 'staff'} systemQty={null} locationName={locationName} onSave={handleNumpadSave} onClose={() => setNumpad({ open: false, product: null })} />)}
    </div>
  );
}
