...
/**
 * Full-screen count sheet for a product counted in crates + loose units.
 * Two steppers (full crates / loose units) with a live base-unit total.
 * Tapping a stepper value opens the shared NumpadModal for direct entry.
 * The parent stores only the base total in Odoo — this sheet just captures
 * the crate/loose split and hands both back via onSave.
 */
...
export default function CrateCountSheet({
  open, product, unitsPerCrate, uom, initialCrates, initialLoose,
  ...
  const total = crateTotal(crates, loose, unitsPerCrate);
    ...
    <div className="fixed inset-0 z-[55] flex flex-col bg-gray-50" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
      ...
      <div className="text-center py-4 px-4">
        ...
        <div className="text-[var(--fs-sm)] text-gray-500 mt-1.5">
          {catLeaf ? `${catLeaf} · ` : ''}
          <span className="font-semibold text-blue-800 bg-blue-50 px-2 py-0.5 rounded-md">1 crate = {unitsPerCrate} {unit}</span>
        </div>
        {showSystemQty && (
      ...
      <div className="flex items-center justify-between gap-3 bg-white border border-gray-200 rounded-2xl px-4 py-3.5 mx-[18px] mt-1">
        <div>
          <div className="text-[var(--fs-md)] font-bold text-gray-900">Full crates</div>
          <div className="text-[var(--fs-xs)] text-gray-500 font-semibold mt-0.5">{'×'} {unitsPerCrate} each</div>
        </div>
        <Stepper value={crates} uom="crates"
      ...
      <div className="flex items-center justify-between gap-3 bg-white border border-gray-200 rounded-2xl px-4 py-3.5 mx-[18px] mt-2.5">
        <div>
          ...
          <div className="text-[var(--fs-xs)] text-gray-500 font-semibold mt-0.5">from opened crates</div>
        </div>
      ...
      <div className="mx-[18px] mt-4 bg-green-50 border border-green-200 rounded-2xl p-4 text-center">
        ...
        <div className="text-[var(--fs-sm)] text-green-700 font-bold mt-1">{unit} total</div>
      </div>
      ...
      <div className="flex gap-2 items-start mx-[18px] mt-3.5 text-[var(--fs-xs)] text-gray-500 leading-snug">
        ...
        <span>Count only full, unopened {unit.toLowerCase()}. A half-empty open one doesn{'’'}t count.</span>
      </div>

      {/* Save */}
      <div className="mt-auto px-[18px] pt-4 pb-6">
        <button onClick={() => onSave(crates, loose)}
      ...
      <NumpadModal
        open={pad !== null}
        productName={pad === 'crates' ? 'Full crates' : `Loose ${unit}`}
        category=""
        uom={pad === 'crates' ? 'crates' : unit}
