...
export default function InventoryDashboard({ userRole, onNavigate, onHome }: InventoryDashboardProps) {
  ...
  async function fetchStats() {
    ...
    try {
      ...
      const pending = sessions.filter((s: { status: string }) => s.status === 'pending' || s.status === 'in_progress').length;
      const submitted = sessions.filter((s: { status: string }) => s.status === 'submitted').length;

      let quickPending = 0;
      if (quickRes) {
        const quickData = await quickRes.json();
        quickPending = (quickData.counts || []).filter((c: { status: string }) => c.status === 'pending').length;
      }
        ...
        <div className="grid grid-cols-2 gap-3">
          {tiles.map((tile) => (
            <button key={tile.id} onClick={() => onNavigate(tile.id)}
              ...
              <div className={`w-11 h-11 rounded-xl ${tile.iconBg || 'bg-gray-100'} ${tile.iconColor || ''} flex items-center justify-center mb-3`}>
                {tile.icon}
              </div>
