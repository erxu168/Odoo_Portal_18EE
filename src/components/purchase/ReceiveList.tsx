...
interface ReceiveListProps {
  ...
  openReceiveCheck: (order: Order) => void;
}

export default function ReceiveList({ pendingDeliveries, openReceiveCheck }: ReceiveListProps) {
  return (
      ...
      {pendingDeliveries.length === 0 ? (
        ...
        pendingDeliveries.map(order => (
          <button key={order.id} onClick={() => openReceiveCheck(order)} className="w-full flex items-center gap-3 p-3.5 bg-white border border-gray-200 rounded-xl shadow-[0_1px_2px_rgba(0,0,0,0.04)] mb-2.5 active:scale-[0.98] transition-transform text-left">
            <div className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center text-[14px] font-bold text-blue-600 flex-shrink-0">{(order.supplier_name || '??').split(' ').map(w => w[0]).join('').slice(0, 2)}</div>
            ...
            <span className="text-[var(--fs-xs)] px-2.5 py-1 rounded-md font-bold bg-amber-100 text-amber-800">{order.status === 'partial' ? 'Partial' : 'Pending'}</span>
          </button>
        ))
      )}
    </div>
