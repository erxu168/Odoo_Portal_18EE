...
import { GuideItem, OdooProduct } from './types';

interface ManageGuideScreenProps {
  guideItems: GuideItem[];
  mgSearch: string;
  mgCategory: string;
  mgResults: OdooProduct[];
  mgCategories: string[];
  mgSearching: boolean;
  mgAdding: number;
  mgConfigOpen: boolean;
  setMgConfigOpen: (v: boolean) => void;
  mgOrderDays: string[];
  setMgOrderDays: React.Dispatch<React.SetStateAction<string[]>>;
  mgDeliveryDays: string[];
  setMgDeliveryDays: React.Dispatch<React.SetStateAction<string[]>>;
  mgLeadTime: number;
  setMgLeadTime: React.Dispatch<React.SetStateAction<number>>;
  mgConfigSaving: boolean;
  saveSupplierConfig: () => void;
  searchProducts: (query: string, category: string) => void;
  handleMgCategoryChange: (cat: string) => void;
  addProductToGuide: (product: OdooProduct) => void;
  removeGuideItemAction: (itemId: number) => void;
  setMgSearch: (v: string) => void;
  setMgResults: (v: OdooProduct[]) => void;
}

export default function ManageGuideScreen({
  guideItems, mgSearch, mgCategory, mgResults, mgCategories, mgSearching, mgAdding,
  mgConfigOpen, setMgConfigOpen, mgOrderDays, setMgOrderDays, mgDeliveryDays, setMgDeliveryDays,
  mgLeadTime, setMgLeadTime, mgConfigSaving, saveSupplierConfig,
  searchProducts, handleMgCategoryChange, addProductToGuide, removeGuideItemAction,
  setMgSearch, setMgResults,
  ...
  return (
    <div className="px-4 py-3">
      ...
      <button onClick={() => setMgConfigOpen(!mgConfigOpen)} className="w-full flex items-center justify-between px-3.5 py-3 bg-white border border-gray-200 rounded-xl shadow-[0_1px_2px_rgba(0,0,0,0.04)] mb-3 active:bg-gray-50">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <div className="text-[var(--fs-xs)] text-gray-400">{mgOrderDays.length > 0 ? `Order: ${mgOrderDays.map(d => d.charAt(0).toUpperCase() + d.slice(1,3)).join(', ')}` : 'Not configured'}{mgDeliveryDays.length > 0 ? ` \u2022 Deliver: ${mgDeliveryDays.map(d => d.charAt(0).toUpperCase() + d.slice(1,3)).join(', ')}` : ''}{mgLeadTime > 1 ? ` \u2022 ${mgLeadTime}d lead` : ''}</div>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-3.5 h-11 focus-within:border-green-500 transition-colors mb-2">
        ...
        <input type="text" value={mgSearch} onChange={e => searchProducts(e.target.value, mgCategory)} placeholder="Search Odoo products to add..." className="flex-1 bg-transparent outline-none text-[var(--fs-base)] text-gray-900 placeholder-gray-400" />
      ...
      {(mgSearch || mgCategory !== 'All') && (
        <div className="mb-4">
          <div className="text-[11px] font-bold tracking-wide uppercase text-gray-400 pb-2">{mgSearching ? 'Searching...' : `${searchResults.length} results`}{searchResults.length > 0 && ' \u2014 tap + to add'}</div>
          {mgSearching && <div className="flex justify-center py-4"><div className="w-6 h-6 border-2 border-gray-300 border-t-green-600 rounded-full animate-spin" /></div>}
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-[0_1px_2px_rgba(0,0,0,0.04)] px-3.5">
              {searchResults.map(product => (
                <div key={product.id} className="flex items-center gap-2.5 py-2.5 border-b border-gray-100 last:border-0">
                  ...
                  <button onClick={() => addProductToGuide(product)} disabled={mgAdding === product.id} className="w-9 h-9 rounded-lg bg-green-500 flex items-center justify-center text-white text-[18px] font-bold shadow-sm active:bg-green-600 flex-shrink-0 disabled:opacity-50">{mgAdding === product.id ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : '+'}</button>
                </div>
              ))}
            </div>
          )}
      ...
      {guideItems.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-xl p-6 text-center"><div className="text-[var(--fs-sm)] text-gray-500">No products yet. Search above to add products from Odoo.</div></div>
      ) : (
        guideCats.map(cat => (
          <div key={cat}>
