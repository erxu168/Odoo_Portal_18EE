    ...
    <div className="px-4 py-3 pb-44">
      <div className="bg-white border border-gray-200 rounded-xl shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-3.5 mb-3">
        ...
        <div className="flex justify-between items-center mt-2 pt-2 border-t border-gray-100">
          <span className="text-[11px] text-gray-400">{cart.item_count} items &bull; {locationName}</span>
          ...
          <div key={item.id} className="flex justify-between items-start py-3 border-b border-gray-100 last:border-0">
            <div className="flex-1 min-w-0 pr-3">
              ...
              <div className="text-[11px] text-gray-500 font-mono mt-0.5">
                {item.quantity} {item.product_uom} &times; &euro;{item.price.toFixed(2)}
              </div>
            </div>
            <div className="text-[13px] font-bold font-mono text-gray-900 flex-shrink-0">
      ...
      <div className="bg-gray-50 rounded-xl px-3.5 py-2.5 mt-2 border border-gray-100">
        <div className="flex justify-between text-[12px] text-gray-500">
          ...
          <span className="font-mono">&euro;{net.toFixed(2)}</span>
        </div>
        {Object.entries(taxByRate)
          .sort(([a], [b]) => Number(a) - Number(b))
          .map(([r, amt]) => (
            <div key={r} className="flex justify-between text-[11px] text-gray-400">
              <span>{r}% MwSt</span>
              <span className="font-mono">&euro;{(amt as number).toFixed(2)}</span>
            </div>
          ))}
        <div className="flex justify-between text-[14px] font-bold text-gray-900 pt-1 border-t border-gray-200 mt-1">
          <span>Total (gross)</span>
          <span className="font-mono">&euro;{gross.toFixed(2)}</span>
        </div>
      </div>
      ...
      <div className="fixed bottom-16 left-0 right-0 max-w-lg mx-auto bg-white border-t border-gray-200 px-4 py-3 z-50">
        <button
          onClick={() => onSend(cart)}
          disabled={sending}
          className="w-full py-3.5 rounded-xl bg-green-600 text-white text-[14px] font-bold shadow-lg shadow-green-600/30 active:bg-green-700 disabled:opacity-50 active:scale-[0.975] transition-all"
        >
          {sending ? 'Sending...' : `Send to ${cart.supplier_name.split(' ')[0]} \u2192`}
        </button>
        {cart.approval_required === 1 && (
          <p className="text-[11px] text-amber-600 text-center mt-1.5">
            This order requires manager approval before sending.
          </p>
        )}
      </div>
    </div>
