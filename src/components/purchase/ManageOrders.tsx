  ...
  return (
    <div className="px-4 py-3">
      ...
      {withProducts.length > 0 && (
        <p className="text-[var(--fs-xs)] text-gray-400 mb-3">Swipe left to delete an order list</p>
      )}

      {suppliers.length === 0 ? (
        <div className="text-center py-8">
          <div className="text-[var(--fs-md)] font-bold text-gray-900 mb-1">No suppliers yet</div>
          <div className="text-[var(--fs-xs)] text-gray-500 mb-4">Search for a supplier below to get started</div>
          {isAdmin && seedMsg && <p className="text-[var(--fs-xs)] text-gray-500 mt-3">{seedMsg}</p>}
        </div>
      ) : (
        <>
          {withProducts.length === 0 && emptyGuides.length === 0 && (
            <div className="text-center py-6 mb-2">
              <div className="text-[var(--fs-md)] font-bold text-gray-900 mb-1">No order lists yet</div>
              <div className="text-[var(--fs-xs)] text-gray-500">Search for a supplier below to create one</div>
            </div>
          )}
          ...
          {withProducts.map((s: any) => (
            <SwipeToDelete key={s.id} onDelete={() => onDeleteGuide(s.id, s.name)}>
              ...
              >
                <div className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center text-[var(--fs-md)] font-bold text-blue-600 flex-shrink-0">
                  {s.name.split(' ').map((w: string) => w[0]).join('').slice(0, 2)}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-[var(--fs-sm)] font-bold text-gray-900 truncate">{s.name}</div>
          ...
          {emptyGuides.length > 0 && (
            <>
              <div className="text-[var(--fs-xs)] font-bold tracking-wide uppercase text-gray-400 pt-4 pb-2">
                Empty lists ({emptyGuides.length})
              </div>
              <p className="text-[var(--fs-xs)] text-gray-400 mb-3">Tap to add products, swipe left to remove</p>
              {emptyGuides.map((s: any) => (
                <SwipeToDelete key={s.id} onDelete={() => onDeleteGuide(s.id, s.name)}>
                  ...
                  >
                    <div className="w-10 h-10 rounded-xl bg-white border border-gray-200 flex items-center justify-center text-[var(--fs-md)] font-bold text-gray-400 flex-shrink-0">
                      {s.name.split(' ').map((w: string) => w[0]).join('').slice(0, 2)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-[var(--fs-sm)] font-semibold text-gray-500 truncate">{s.name}</div>
                      <div className="text-[var(--fs-xs)] text-gray-400">No products yet {'\u2022'} Tap to add</div>
                    </div>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#D1D5DB" strokeWidth="2">
                      <path d="M9 5l7 7-7 7"/>
                    </svg>
      ...
      <div className="mt-4 pt-4 border-t border-gray-200">
        {!showSearch ? (
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-green-50 border border-green-200 text-green-700 text-[var(--fs-sm)] font-semibold active:bg-green-100 transition-colors"
          >
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <div className="flex-1 flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-3.5 h-11 focus-within:border-green-500 transition-colors">
                ...
                <input
                  ...
                  className="flex-1 bg-transparent outline-none text-[var(--fs-sm)] text-gray-900 placeholder-gray-400"
                />
                {searchQuery && (
                  <button onClick={() => { setSearchQuery(''); setSearchResults([]); }} className="text-gray-400 text-[var(--fs-lg)]">&times;</button>
                )}
              </div>
              <button onClick={closeSearch} className="text-[var(--fs-xs)] font-semibold text-gray-500 px-2 py-2">
                Cancel
              </button>
            </div>

            {!searching && searchQuery.length === 0 && (
              <div className="text-[var(--fs-xs)] text-gray-400 text-center py-4">Type at least 2 characters to search Odoo suppliers</div>
            )}

            {!searching && searchQuery.length === 1 && (
              <div className="text-[var(--fs-xs)] text-gray-400 text-center py-4">Keep typing...</div>
            )}
            ...
            {searchError && (
              <div className="text-[var(--fs-xs)] text-red-500 text-center py-4">{searchError}</div>
            )}

            {!searching && searchQuery.length >= 2 && searchResults.length === 0 && !searchError && (
              <div className="text-[var(--fs-xs)] text-gray-500 text-center py-6">No suppliers found matching &quot;{searchQuery}&quot;</div>
            )}
            ...
            {searchResults.length > 0 && (
              <div className="bg-white border border-gray-200 rounded-xl shadow-[0_1px_2px_rgba(0,0,0,0.04)] overflow-hidden">
                {searchResults.map((partner: any) => (
                  ...
                  >
                    <div className="w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center text-[var(--fs-xs)] font-bold text-blue-600 flex-shrink-0">
                      {partner.name.split(' ').map((w: string) => w[0]).join('').slice(0, 2)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-[var(--fs-sm)] font-semibold text-gray-900 truncate">{partner.name}</div>
                      {partner.email && <div className="text-[var(--fs-xs)] text-gray-400 truncate">{partner.email}</div>}
                    </div>
