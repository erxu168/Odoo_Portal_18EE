  ...
  useEffect(() => {
    if (sqcEnabled && drivingComponent) {
      ...
      if (!isNaN(manualQty) && manualQty > 0 && drivingComponent.required_qty > 0) {
        ...
        const drivingRatio = drivingComponent.required_qty > 0 ? drivingComponent.on_hand_qty / drivingComponent.required_qty : 0;
        setQty(String(Math.floor(drivingRatio * baseQty)));
      }
    }
    ...
    try {
      const res = await fetch('/api/manufacturing-orders', {
        ...
        body: JSON.stringify({
          ...
          bom_id: bom.id, product_qty: numQty,
          product_uom_id: bom.product_uom_id[0], date_deadline: scheduledDate,
        }),
      });
      const data = await res.json();
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="pt-2">
        ...
        <div className="px-5 pb-3">
          ...
          <div className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-white text-[14px] font-semibold text-gray-900">
            {bom.product_tmpl_id[1]}
          </div>
        </div>
        ...
        <div className="mx-4 mb-3 flex items-center justify-between bg-white border border-gray-200 rounded-xl px-4 py-3">
          ...
          <button
            type="button"
            onClick={() => { setSqcEnabled(!sqcEnabled); if (sqcEnabled) { setQty(String(baseQty)); setDrivingComponentQty(''); } }}
        ...
        {sqcEnabled && (
          <div className="px-5 pb-3">
            ...
            <select
              ...
              onChange={(e) => { setDrivingComponentId(parseInt(e.target.value) || null); setDrivingComponentQty(''); }}
            ...
            >
              ...
              {components.map((c) => (
                <option key={c.product_id} value={c.product_id}>{c.product_name} ({c.uom})</option>
              ))}
            </select>

            {drivingComponentId && (
              <div>
                <label className="text-[12px] font-semibold text-gray-400 tracking-wider uppercase mb-1.5 block">
                ...
                <div className="flex items-center border border-orange-200 rounded-xl bg-white overflow-hidden">
                  <input
                    type="number" inputMode="decimal" value={drivingComponentQty}
                  ...
                  />
                  <div className="px-3 py-3 text-sm text-gray-500 bg-gray-50 border-l border-gray-200">
        ...
        <div className="mx-5 mb-1.5 flex items-center border border-gray-200 rounded-xl bg-white overflow-hidden">
          <input
            ...
            onChange={(e) => setQty(e.target.value)} readOnly={sqcEnabled}
        ...
        <div className="px-5 pb-2.5 text-[11px] text-gray-400">
          ...
          {sqcEnabled && drivingComponent ? ` \u00b7 Based on ${drivingComponent.product_name}` : ''}
        </div>
        ...
        <div className="px-5 pt-1 pb-2">
          <p className="text-[11px] font-semibold text-gray-400 tracking-widest uppercase">
            Scaled ingredients ({new Intl.NumberFormat('de-DE').format(numQty)}{uom})
          </p>
        </div>
          ...
          {scaledComponents.map((c) => (
            <div key={c.product_id}
              className={`bg-white border rounded-xl px-4 py-3 flex justify-between items-center ${
                sqcEnabled && c.product_id === drivingComponentId ? 'border-orange-200' : 'border-gray-200'
              }`}>
              <span className="text-[13px] text-gray-900 flex items-center gap-1.5">
                ...
                {sqcEnabled && c.product_id === drivingComponentId && (
                  <span className="text-[11px] text-orange-600 font-semibold">(driving)</span>
                )}
              </span>
              <div className="flex gap-4 items-baseline">
                <span className={`text-[13px] font-bold font-mono ${c.is_short ? 'text-red-600' : 'text-gray-900'}`}>
                  {new Intl.NumberFormat('de-DE').format(c.scaled_qty)}{c.uom}
                </span>
                <span className={`text-[11px] ${c.is_short ? 'text-red-500' : 'text-emerald-600'}`}>
                  {new Intl.NumberFormat('de-DE').format(c.on_hand_qty)} {c.uom} avail
                </span>
              </div>
            </div>
          ))}
      ...
      {submitError && (
        <div className="mx-4 mb-2 px-4 py-3 rounded-xl bg-red-50 border border-red-200 text-[13px] text-red-700">{submitError}</div>
      )}
      {draftSaved && (
        <div className="mx-4 mb-2 px-4 py-3 rounded-xl bg-emerald-50 border border-emerald-200 text-[13px] text-emerald-700">
      ...
      <div className="px-4 pb-6 pt-3">
        ...
        <div className="mt-2">
          <ActionButton variant="outline" onClick={handleSaveDraft} disabled={submitting || numQty <= 0}>
            Save as draft
          </ActionButton>
        </div>
      </div>
