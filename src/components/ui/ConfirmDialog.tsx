...
interface ConfirmDialogProps {
  ...
  /** Optional control shown between the message and the buttons — e.g. a
   *  "move them to…" picker, so a blocked action offers a way forward
...
export default function ConfirmDialog({ title, message, confirmLabel, cancelLabel = 'Cancel', variant = 'primary', extra, onConfirm, onCancel, onDismiss }: ConfirmDialogProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-[110] flex items-end justify-center" onClick={onDismiss || onCancel}>
      <div className="bg-white rounded-t-[20px] w-full max-w-lg p-5 pb-7" onClick={e => e.stopPropagation()}>
        ...
        <p className="text-[13px] text-gray-500 leading-relaxed mb-4">{message}</p>
