...
interface Props {
  ...
  featuredSource?: 'manual' | 'auto';
  onSelectFeatured?: (tile: FeaturedTile) => void;
}

export default function ActiveSessions({ sessions, onSelectSession, onNewDish, onBack, onHome, onEndSession, featured = [], featuredSource = 'manual', onSelectFeatured }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[#1C1C1E] flex flex-col">
      ...
      <div className="flex-1 px-3 pt-3 pb-6 overflow-y-auto">
        {/* Featured / quick-start dishes — tap straight to cook */}
        {featured.length > 0 && (
          <div className="mb-4">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {featured.map((t) => (
                <button
                  key={`${t.mode}-${t.recipe_id}`}
                  onClick={() => onSelectFeatured?.(t)}
                  className={`rounded-2xl border-2 p-3 text-left active:opacity-80 transition-colors min-h-[64px] flex items-center gap-2.5 ${t.mode === 'production' ? 'border-blue-500/40 bg-blue-500/10' : 'border-green-500/40 bg-green-500/10'}`}
                >
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${t.mode === 'production' ? 'bg-blue-500/20' : 'bg-green-500/20'}`}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={t.mode === 'production' ? '#60a5fa' : '#4ade80'} strokeWidth="2"><path d="M4 19.5A2.5 2.5 0 016.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z"/></svg>
                  </div>
                  <span className="text-[13px] font-bold text-white leading-tight line-clamp-2">{t.recipe_name}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {active.length === 0 && featured.length === 0 && (
          <div className="text-center py-20">
            ...
            <div className="text-[16px] font-bold text-zinc-200 mb-2">Nothing to prepare yet</div>
            <div className="text-[13px] text-zinc-400 mb-6 px-8 leading-relaxed">A manager adds dishes here by opening a recipe and tapping the {'★'} star.<br/>Or tap below to start any recipe now.</div>
