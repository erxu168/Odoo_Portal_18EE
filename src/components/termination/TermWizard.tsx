...
export default function TermWizard({ onBack, onCreated, onHome, preselectEmployeeId }: TermWizardProps) {
  ...
  const departments = useMemo(() => {
    ...
    for (const emp of employees) {
      if (emp.department_id) deptMap.set(emp.department_id[0], emp.department_id[1]);
    }
  ...
  const filteredEmployees = useMemo(() => {
    return employees.filter(e => {
      ...
      if (deptFilter !== null && (!e.department_id || e.department_id[0] !== deptFilter)) return false;
      return true;
    });
  ...
  function buildFormBody(): Record<string, unknown> {
    ...
    const body: Record<string, unknown> = {
      employee_id: selectedEmployee.id, company_id: selectedEmployee.company_id[0],
      termination_type: termType, calc_method: calcMethod, letter_date: letterDate,
    ...
    if (termType === 'bestaetigung' && resignationDate) body.resignation_received_date = resignationDate;
    return body;
  }
  ...
  async function handleGeneratePreview() {
    ...
    try {
      ...
      const createRes = await fetch('/api/termination', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
  ...
  const formatDate = (d: string | false) => { if (!d) return '\u2013'; const [y, m, day] = d.split('-'); return `${day}.${m}.${y}`; };
    ...
    return (
      <div className="space-y-4">
        ...
        <button onClick={handleGeneratePreview} disabled={previewLoading}
          ...
          {previewLoading ? 'Generating preview...' : 'Preview letter'}</button>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-4">
        ...
        {pdfBase64 && (
          <div className="flex gap-2">
            <button onClick={() => setShowPdfViewer(true)}
              className="flex-1 py-3.5 rounded-xl bg-white border-2 border-red-200 text-red-700 font-semibold text-[14px] flex items-center justify-center gap-2 active:bg-red-50">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
              View
            </button>
            <button onClick={handlePrintPreview}
              className="flex-1 py-3.5 rounded-xl bg-white border-2 border-gray-200 text-gray-700 font-semibold text-[14px] flex items-center justify-center gap-2 active:bg-gray-50">
        ...
        <button onClick={handleConfirm} disabled={submitting}
          className="w-full py-3.5 rounded-xl bg-red-600 text-white font-semibold text-[15px] active:bg-red-700 disabled:opacity-50">
          {submitting ? 'Creating...' : 'Confirm & create termination'}</button>
        ...
        {showPdfViewer && pdfBase64 && (
          <PdfViewer fileData={pdfBase64} fileName="Termination_Draft.pdf" onClose={() => setShowPdfViewer(false)} />
        )}
      </div>
    );
  }

  const stepLabels: Record<Step, string> = { employee: '1. Select employee', type: '2. Termination type', details: '3. Details', preview: '4. Review & confirm' };
  ...
  return (
    <div>
      <AppHeader
        ...
        showBack
        onBack={step === 'employee' ? onBack : () => {
          if (step === 'type') setStep('employee');
          else if (step === 'details') setStep('type');
          else if (step === 'preview') handleDiscard();
        }}
      />
      <div className="bg-[#2563EB] px-5 pb-3 -mt-2 rounded-b-[28px]">
        <div className="flex gap-1">
          {(['employee', 'type', 'details', 'preview'] as Step[]).map((s, i) => (
            <div key={s} className={`flex-1 h-1 rounded-full ${i <= ['employee', 'type', 'details', 'preview'].indexOf(step) ? 'bg-white' : 'bg-white/20'}`} />
          ))}
        </div>
      </div>
