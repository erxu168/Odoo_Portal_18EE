...
export default function TermDashboard({ onNavigate }: TermDashboardProps) {
  ...
  const tiles = [
    {
      key: 'new', label: 'New Termination', sublabel: 'Create',
      ...
      badge: null, color: 'bg-green-50 border-green-200', iconBg: 'bg-green-100', iconColor: 'text-green-600',
    },
    {
      key: 'drafts', label: 'Drafts', sublabel: 'Open',
      ...
      badge: stats.draft > 0 ? stats.draft : null, color: 'bg-amber-50 border-amber-200', iconBg: 'bg-amber-100', iconColor: 'text-amber-600',
    },
    {
      key: 'confirmed', label: 'Confirmed', sublabel: 'Active',
      ...
      badge: stats.confirmed > 0 ? stats.confirmed : null, color: 'bg-blue-50 border-blue-200', iconBg: 'bg-blue-100', iconColor: 'text-blue-600',
    },
    {
      key: 'archive', label: 'Archive', sublabel: 'Completed',
      icon: (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="21 8 21 21 3 21 3 8"/><rect x="1" y="3" width="22" height="5"/><line x1="10" y1="12" x2="14" y2="12"/></svg>),
      badge: stats.archived + stats.signed > 0 ? stats.archived + stats.signed : null, color: 'bg-gray-50 border-gray-200', iconBg: 'bg-gray-100', iconColor: 'text-gray-500',
    },
  ];
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-3">
            {tiles.map(tile => (
              <button key={tile.key} onClick={() => onNavigate(tile.key)}
                className={`relative rounded-2xl border ${tile.color} shadow-sm p-4 text-left active:scale-[0.97] transition-transform`}>
                <div className={`w-11 h-11 rounded-xl ${tile.iconBg} ${tile.iconColor} flex items-center justify-center mb-3`}>{tile.icon}</div>
                ...
                {tile.badge !== null && (<span className="absolute top-3 right-3 min-w-[20px] h-5 px-1.5 rounded-full bg-red-500 text-white text-[11px] font-bold flex items-center justify-center">{tile.badge}</span>)}
              </button>
            ))}
          </div>
          {recent.length > 0 && (
            <div className="mt-6">
              <div className={`${ds.sectionLabel} mb-3`}>Recent</div>
              <div className="space-y-2">
                {recent.map((r: any) => (
                  <button key={r.id} onClick={() => onNavigate(`detail:${r.id}`)} className={`${ds.cardHover} w-full text-left p-3.5`}>
