...
export default function SharedTabletsSection() {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-4 mb-4">
      <h2 className="text-[15px] font-bold text-gray-900 mb-1">Tablet access</h2>
      <p className="text-[13px] text-gray-500 mb-3">Every set-up tablet. Name them so you can tell them apart, turn access on/off, or remove a lost or unused one.</p>
            ...
            <div key={t.id} className="flex items-center gap-3 py-3">
              <div className="min-w-0 flex-1">
                <button onClick={() => rename(t)} disabled={busy === t.id} className="flex items-center gap-1.5 max-w-full text-left active:opacity-70 disabled:opacity-50" aria-label="Rename this tablet">
                  <span className={`text-[14px] font-semibold truncate ${t.name ? 'text-gray-900' : 'text-amber-700'}`}>{t.name || 'Name this tablet'}</span>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`flex-shrink-0 ${t.name ? 'text-gray-400' : 'text-amber-500'}`}><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>
                </button>
                <div className="text-[12px] text-gray-500 truncate">
                  {t.label ? `${t.label} · ` : ''}set up{t.created_by ? ` by ${t.created_by}` : ''} {fmt(t.created_at)} · last used {fmt(t.last_used_at)}
                </div>
              </div>
