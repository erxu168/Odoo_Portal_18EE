...
export default function EditProperty() {
  ...
  useEffect(() => {
    async function fetchProperty() {
      try {
        const res = await fetch(`/api/rentals/properties/${id}`);
        ...
        setPlz(p.plz || '');
        setCity(p.city || '');
        setFloorUnit(p.floor_unit || '');
        setType(p.type || 'apartment_wg');
        setTotalSize(p.total_size_sqm != null ? String(p.total_size_sqm) : '');
        setOwner(p.owner || '');
        setHausverwaltung(p.hausverwaltung || '');
        setMietspiegel(p.mietspiegel_eur_per_sqm != null ? String(p.mietspiegel_eur_per_sqm) : '');
        setRundfunkNo(p.rundfunkbeitragsnummer || '');
        setNotes(p.notes || '');
      } catch (err) {
        console.error('[rentals] fetch property failed:', err);
        alert('Failed to load property');
        router.push(`/rentals/properties/${id}`);
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleSave() {
    ...
    try {
      ...
      console.error('[rentals] edit property failed:', err);
      alert('Network error');
      setSaving(false);
    }
  }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F6F7F9]">
        <AppHeader title="Edit Property" supertitle="RENTALS" showBack onBack={() => router.push(`/rentals/properties/${id}`)} />
    ...
    <div className="min-h-screen bg-[#F6F7F9]">
      ...
      {showConfirm && (
        <ConfirmDialog
          ...
          variant="danger"
          onConfirm={() => router.push(`/rentals/properties/${id}`)}
