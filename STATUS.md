...
| Cooking Timer | ✅ shipped (staging) | station-based multi-step cooking timers for WAJ. Cook tablet `/cooktimer` (no-login, immersive) + manager Setup `/cooktimer-setup` (manager-gated, `cooktimer.config.manage`). Shared KDS POS feed, server-owned state machine, per-line ✓ back onto the KDS. Steps 0–5 done: DB, API, station screen, KDS integration, and the Setup screen (mock signed off 2026-07-28 — max batch **enforced** by splitting oversized COOK ALLs into several baskets, live till dish names, move-dishes-on-station-delete). Android APK: `cooktimer-latest` release. **Real cook TIMES are still placeholders — measure on the floor and enter them here.** Doc drift: spec references `src/lib/ux-rules.ts`, absent from the repo; spec's `/cooktimer/profiles` route is really `/cooktimer-setup` (kept outside the public `/cooktimer` prefix). |
